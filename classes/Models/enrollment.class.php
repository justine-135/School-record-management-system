...
class Enrollment extends \Dbh{
    ...
    $father_education_textbox, $mother_education_textbox, $guardian_education_textbox)
    {
        try{
            ...
            // Query
            $sql = 
            "INSERT INTO `students_table` (surname, first_name, middle_name, ext, lrn,
            grade_level, from_sy, to_sy, birth_date, gender, religion, house_street, subdivision, barangay, city, province, region)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?);
            INSERT INTO `enrollment_history_table` (student_lrn, from_sy, to_sy, school, grade, `status`)
            VALUES (?, ?, ?, ?, ?, ?);
            ";
            // Execute
            $stmt = $this->connection()->prepare($sql);
            $stmt->execute([$sname, $fname, $mname, $extname, $lrn, $grade_lvl, $from_sy, $to_sy, $bdate, $gender, $religion, $house_street, $subdivision, $barangay, $city, $province, $region,
            ...
            $lrn, $from_sy, $to_sy, $school, $grade_lvl, $status]);
            $stmt = null;
